...
import { Calendar, Clock, Tag, CheckSquare, Square, Edit, Trash2, AlertCircle } from 'lucide-react';
import { format } from 'date-fns';
import { useTodoStore } from './todo-store';

// Get types from store
type Task = Parameters<ReturnType<typeof useTodoStore>['updateTask']>[1] & { id: string };
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, isDragging = false }) => {
  ...
  const getPriorityIcon = (priority: string) => {
    const size = 12;
    const className = priority === 'high' ? 'text-red-500' : 
  ...
  if (isDragging || isSortableDragging) {
    return (
      <div
        ref={setNodeRef}
        style={style}
        className={`p-4 rounded-lg border-l-4 shadow-lg ${getPriorityColor(task.priority)} ${
          settings.darkMode ? 'bg-gray-800 border border-gray-600' : 'bg-white border border-gray-200'
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`group p-4 rounded-lg border-l-4 cursor-grab active:cursor-grabbing transition-all duration-200 hover:shadow-md ${
