...
const ListView: React.FC<ListViewProps> = ({ tasks, onEditTask }) => {
  ...
  const handleSortChange = (sortBy: typeof settings.listSortBy) => {
    const newOrder = settings.listSortBy === sortBy && settings.listSortOrder === 'desc' ? 'asc' : 'desc';
    updateSettings({ 
      listSortBy: sortBy,
  ...
  const getSortIcon = (sortBy: typeof settings.listSortBy) => {
    ...
    return settings.listSortOrder === 'asc' ? <SortAsc size={16} /> : <SortDesc size={16} />;
  };
  ...
  const getGroupColor = (groupKey: string) => {
    if (settings.listGroupBy === 'priority') {
      ...
      if (groupKey.includes('Medium')) return 'text-yellow-600 border-yellow-200';
    ...
    return settings.darkMode ? 'text-gray-300 border-gray-600' : 'text-gray-700 border-gray-200';
  };

  return (
    <div className="h-full flex flex-col">
      {/* List Controls */}
      <div className={`p-4 border-b ${settings.darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
            ...
            {sortedGroupKeys.map((groupKey) => {
              ...
              return (
                <div key={groupKey} className="space-y-4">
                  {/* Group Header */}
                  <div className={`flex items-center space-x-2 pb-2 border-b ${getGroupColor(groupKey)}`}>
                  ...
                  <div className="grid gap-4">
                    {groupTasks.map((task) => (
                      <div key={task.id} className="transition-all duration-200 hover:scale-[1.02]">
