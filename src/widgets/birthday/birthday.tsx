    ...
    const createConfetti = () => {
      ...
      setTimeout(() => {
        setConfettiPieces((prev) =>
          prev.filter(
            (piece) => !newPieces.some((newPiece) => newPiece.id === piece.id)
          )
        );
      }, 3000);
    };
    ...
    >
      ...
      <div className="text-center p-8 max-w-4xl mx-auto relative z-10">
        <div className={`${config.animation} ${config.font}`}>
          <h1
            className={`text-6xl md:text-8xl font-bold mb-4 ${config.theme.primary} drop-shadow-lg transition-all duration-300 hover:scale-105 hover:text-yellow-300 cursor-default`}
          >
            <TypingAnimation text={config.message} speed={150} />
          </h1>
          <h2
            className={`text-4xl md:text-6xl font-semibold ${config.theme.secondary} drop-shadow-md transition-all duration-300 hover:scale-110 hover:rotate-1 cursor-default`}
          >
        ...
        <div className="mt-8 space-y-4">
          <div className="text-2xl md:text-3xl">
            <span className="inline-block animate-spin text-4xl hover:animate-bounce cursor-pointer transition-all duration-300 hover:scale-125">
              🎂
            </span>
            <span className="mx-4 animate-pulse text-4xl hover:animate-spin cursor-pointer transition-all duration-300 hover:scale-125">
              🎈
            </span>
            <span className="inline-block animate-bounce text-4xl hover:animate-pulse cursor-pointer transition-all duration-300 hover:scale-125">
              🎁
            </span>
          </div>
          <p
            className={`text-lg md:text-xl ${config.theme.text} opacity-90 max-w-2xl mx-auto hover:opacity-100 transition-opacity duration-300`}
          >
